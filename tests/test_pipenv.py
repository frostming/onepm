...
def test_pipenv_pi(project, execute_command, args):
    pi(args)
    execute_command.assert_called_with(["pipenv", "install", *args], None)


def test_pipenv_pr(project, execute_command):
    pr(["test", "--no-report"])
    execute_command.assert_called_with(["pipenv", "run", "test", "--no-report"], None)


def test_pipenv_pu(project, execute_command):
    pu(["requests"])
    execute_command.assert_called_with(["pipenv", "update", "requests"], None)


def test_pipenv_pun(project, execute_command):
    pun(["-d", "requests"])
    execute_command.assert_called_with(["pipenv", "uninstall", "-d", "requests"], None)
